...
def posterior_expectation(model,posterior_samples, posterior_weights, inputs ):
    ...
    for sample_idx in range(num_posterior_samples):
        ...
        outputs_weighted_sum = outputs_weighted_sum + outputs_samples*posterior_weights[sample_idx]
   ...
   # Training
    for epoc in range(50):
        for i, data in enumerate(trainloader, 0):
            ...
            loss_monitor.record_tensorboard(loss.data[0],iteration,sess,train_writer)
            ...
            if is_collecting == True and i%sample_interval == 0:
                posterior_samples, posterior_weights = posterior_sampling(sample_size, model, learning_rate, posterior_samples, posterior_weights)
            ...
            if (iteration % validation_interval == 0) and (len(posterior_samples) == sample_size):
                ...
                # Bayesian Estimation
                posterior_outputs = posterior_expectation(model, posterior_samples, posterior_weights, test_inputs)
